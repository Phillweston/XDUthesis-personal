...
目前以DJI为主的商业公司已具备完善的软硬件体系，但由于其商业性质较浓厚，缺乏对于开源协议和开源地面站的兼容。与此同时，Ardupilot和PX4作为飞控开源界最大的两个项目，采用QGroundControl作为地面站软件并通过\textbf{MAVLink协议}实现空地间的数据传输。因此，市面上迫切需要一种解决方案将开源地面站的相关功能与DJI飞行器MSDK接口进行结合，并通过MAVLink协议开放其飞行控制接口。
